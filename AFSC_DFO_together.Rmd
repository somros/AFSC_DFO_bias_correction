...
1. R>0.5, good convergence, range > 20 km, 1 for visual
2. R>0.5, good convergence, range > 20 km, 2 for visual
3. R<0.5 or convergence issues or range issues and 2, or 3 for visual
4. Minimal data
5. No convergence - discard

This was done for AKFIN and DFO separately, then the scores were added for each group. Based on the resulting score, we decide whether or not we apply the bias correction (and whether we can trus the models at all). 
...
```{r}
val <- readxl::read_xlsx("validation.xlsx", "GOA", col_types = c("text",rep("numeric",3)))

val_groups <- val %>% drop_na() %>% filter(GOA < 7) %>% select(Group) %>% pull()
```
...
write.csv(s1_s4_wide_final, "S1-S4.csv", row.names = FALSE)
```
